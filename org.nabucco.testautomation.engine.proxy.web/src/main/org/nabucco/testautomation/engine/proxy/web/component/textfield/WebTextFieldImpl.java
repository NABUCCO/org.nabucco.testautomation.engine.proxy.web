...
public class WebTextFieldImpl extends AbstractWebComponent {
    ...
    protected ActionResponse internalExecute(TestContext context, PropertyList propertyList,
        ...
        try {
        	...
        	String errorMessage = "Could not execute WebTextField-command. Cause: " + ex.getMessage();
        	...
        	this.fatal(ex);
            result.setErrorMessage("Could not execute WebTextField-command. Cause: " + ex.toString());
            result.setActionStatus(ActionStatusType.FAILED);
            return result;
        } finally {
