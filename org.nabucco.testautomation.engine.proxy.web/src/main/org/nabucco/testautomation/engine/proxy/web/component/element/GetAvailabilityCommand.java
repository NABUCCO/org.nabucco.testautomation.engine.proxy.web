 ...
 */
public class GetAvailabilityCommand extends AbstractWebComponentCommand {

	/**
	 * @param selenium
	 */
	public GetAvailabilityCommand(Selenium selenium) {
		super(selenium);
	}
	...
	public PropertyList executeCallback(Metadata metadata,
		...
		BooleanProperty property = (BooleanProperty) PropertyHelper
				.getFromList(properties, PropertyType.BOOLEAN).cloneObject();
		this.checkTimeout(properties);

		this.start();
		property.setValue(this.waitForElement(element));
		this.stop();

		PropertyList returnProperties = PropertyHelper
				.createPropertyList(RETURN_PROPERTIES);
