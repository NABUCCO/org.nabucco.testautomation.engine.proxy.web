...
*/
package org.nabucco.testautomation.engine.proxy.web.component.menu;
 ...
 */
public class WebContextMenuImpl extends AbstractWebComponent {
	 ...
	 * @param selenium
	 */
	public WebContextMenuImpl(Selenium selenium) {
		super(selenium);
	}
	...
	protected ActionResponse internalExecute(TestContext context,
        ...
        try {
			...
			case OPEN:
				command = new OpenContextMenuCommand(this.getSelenium());
				break;
				
			default:
				return failResult(metadata, actionType, "Unsupported WebActionType for WebContextMenu: '" + actionType + "'");
			}
			
			// Execute WebCommand
			PropertyList returnProperties = command.execute(metadata, propertyList);
			
			result.setMessage("Executed WebContextMenu action='" + actionType + "'");
        	...
        	String errorMessage = "Could not execute WebContextMenu-command. Cause: " + ex.getMessage();
        	...
        	this.fatal(ex);
            result.setErrorMessage("Could not execute WebContextMenu-command. Cause: " + ex.toString());
            result.setActionStatus(ActionStatusType.FAILED);
            return result;
        } finally {
