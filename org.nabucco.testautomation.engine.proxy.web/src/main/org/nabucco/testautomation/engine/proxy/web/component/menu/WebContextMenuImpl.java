...
public class WebContextMenuImpl extends AbstractWebComponent {
    ...
    protected ActionResponse internalExecute(TestContext context, PropertyList propertyList, Metadata metadata,
        ...
        try {
            ...
            default:
                return failResult(metadata, actionType, "Unsupported WebActionType for WebContextMenu: '"
                        + actionType + "'");
            }
            ...
            String errorMessage = "Could not execute "
                    + actionType + " on WebContextMenu '" + metadata.getName().getValue() + "'. Cause: "
                    + ex.getMessage();
            ...
            result.setErrorMessage("Could not execute "
                    + actionType + " on WebContext '" + metadata.getName().getValue() + "'. Cause: " + ex.toString());
            result.setActionStatus(ActionStatusType.FAILED);
            return result;
        } finally {
