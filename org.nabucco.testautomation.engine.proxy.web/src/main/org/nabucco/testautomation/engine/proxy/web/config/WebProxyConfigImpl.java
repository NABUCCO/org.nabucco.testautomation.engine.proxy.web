 ...
 * 
 */
public class WebProxyConfigImpl extends AbstractProxyEngineConfiguration implements WebProxyConfiguration {

    private static final int DEFAULT_SELENIUM_RC_TIMEOUT = 30;

    private static final int DEFAULT_SELENIUM_PORT = 4444;

    private static final boolean DEFAULT_SELENIUM_TRUST = false;
     ...
     */
    public WebProxyConfigImpl(ClassLoader classloader, ProxyConfiguration configuration) {
    ...
    public int getServerPort() {
        try {
            ...
            System.err.println("Cannot parse Property name='"
                    + WebProxyConfigurationType.SERVER_PORT.getKey() + "'. using default 4444");
    ...
    public boolean getTrustAllSSLCertificates() {
        try {
            boolean trust = Boolean
                    .parseBoolean(getConfigurationValue(WebProxyConfigurationType.TRUST_ALL_SSL_CERTIFICATES.getKey()));
            return trust;
        } catch (Exception e) {
            System.err.println("Cannot parse Property name='"
                    + WebProxyConfigurationType.TRUST_ALL_SSL_CERTIFICATES.getKey() + "'. using default 'false'");
            return DEFAULT_SELENIUM_TRUST;
        }
    }
