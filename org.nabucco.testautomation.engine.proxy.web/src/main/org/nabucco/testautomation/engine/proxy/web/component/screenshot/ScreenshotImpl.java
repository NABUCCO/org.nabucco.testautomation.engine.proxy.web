 ...
 */
public class ScreenshotImpl extends AbstractWebComponent {
	 ...
	 * @param selenium
	 */
	public ScreenshotImpl(Selenium selenium) {
		super(selenium);
	}
	...
	protected ActionResponse internalExecute(TestContext context,
        ...
        try {
	        ...
	        case CREATE:
	            command = new CreateScreenshotCommand(this.getSelenium());
	            break;
	
	        default:
	            return failResult(metadata, actionType, "Unsupported WebActionType for Screenshot: '" + actionType + "'");
	        }
	        
	        // Execute WebCommand
			PropertyList returnProperties = command.execute(metadata, propertyList);
			
			result.setMessage("Executed Screenshot action='" + actionType + "'");
        	...
        	String errorMessage = "Could not execute Screenshot-command. Cause: " + ex.getMessage();
        	...
        	this.fatal(ex);
            result.setErrorMessage("Could not execute Screenshot-command. Cause: " + ex.toString());
            result.setActionStatus(ActionStatusType.FAILED);
            return result;
        } finally {
        	
        	// Always set ActionTrace as it contains the screenshot
        	if (command != null) {
