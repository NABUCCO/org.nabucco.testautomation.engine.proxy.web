...
public class EvaluateJavaScriptCommand extends AbstractWebComponentCommand {
	...
	private String getJavaScript(Metadata metadata, PropertyList properties) throws WebComponentException {
		...
		StringProperty prop = (StringProperty) PropertyHelper.getFromList(properties, PropertyType.STRING, JAVA_SCRIPT);

		if (prop != null) {
			return prop.getValue().getValue();
		}
		
		prop = (StringProperty) PropertyHelper.getFromList(metadata.getPropertyList(), PropertyType.STRING, JAVA_SCRIPT);
