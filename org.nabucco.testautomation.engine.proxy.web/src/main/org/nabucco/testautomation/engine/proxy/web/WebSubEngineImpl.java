...
import org.nabucco.testautomation.engine.proxy.web.WebEngine;
import org.nabucco.testautomation.engine.proxy.web.WebEngineOperationType;
...
public class WebSubEngineImpl extends AbstractSubEngine implements WebEngine {
	...
	private static final NBCTestLogger logger = NBCTestLoggingFactory
			.getInstance().getLogger(WebSubEngineImpl.class);
	...
	public ActionResponse executeSubEngineOperation(
		...
		case WEB_BUTTON: {
			return this.getWebButton().execute(context, propertyList,
					metadataList, actionType);
		}
		case WEB_CHECKBOX: {
			return this.getWebCheckbox().execute(context, propertyList,
					metadataList, actionType);
		}
		case WEB_DROPDOWNBOX: {
			return this.getWebDropDownBox().execute(context, propertyList,
					metadataList, actionType);
		}
		case WEB_HYPERLINK: {
			return this.getWebHyperlink().execute(context, propertyList,
					metadataList, actionType);
		}
		case WEB_CAPTCHA: {
			return this.getWebCaptcha().execute(context, propertyList,
					metadataList, actionType);
		}
		case WEB_PAGE: {
			return this.getWebPage().execute(context, propertyList,
					metadataList, actionType);
		}
		case WEB_TEXTFIELD: {
			return this.getWebTextField().execute(context, propertyList,
					metadataList, actionType);
		}
		case WEB_DOCUMENT: {
			return this.getWebDocument().execute(context, propertyList,
					metadataList, actionType);
		}
		case WEB_ELEMENT: {
			return this.getWebElement().execute(context, propertyList,
					metadataList, actionType);
		}
		case WEB_CONTEXT_MENU: {
			return this.getWebContextMenu().execute(context, propertyList,
					metadataList, actionType);
		}
		case WEB_DIALOG: {
			return this.getWebDialog().execute(context, propertyList,
					metadataList, actionType);
		}
		case WEB_TABLE: {
			return this.getWebTable().execute(context, propertyList,
					metadataList, actionType);
		}
		case BROWSER_WINDOW: {
			return this.getBrowserWindow().execute(context, propertyList,
					metadataList, actionType);
		}
		case COOKIE: {
			return this.getCookie().execute(context, propertyList,
					metadataList, actionType);
		}
		case SCREENSHOT: {
			return this.getScreenshot().execute(context, propertyList,
					metadataList, actionType);
		}
		case JAVA_SCRIPT: {
			return this.getJavaScript().execute(context, propertyList,
					metadataList, actionType);
		}
		default: {
			String error = "Unsupported WebEngineOperationType = '"
					+ operationType + "'";
			logger.error(error);
