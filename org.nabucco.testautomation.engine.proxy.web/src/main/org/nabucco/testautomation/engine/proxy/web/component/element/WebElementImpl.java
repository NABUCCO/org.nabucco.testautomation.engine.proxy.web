...
public class WebElementImpl extends AbstractWebComponent {
    ...
    protected ActionResponse internalExecute(TestContext context, PropertyList propertyList, Metadata metadata,
        ...
        try {
            ...
            case MOUSEOVER:
                command = new MouseOverCommand(this.getSelenium());
                break;

            case MOUSEOUT:
                command = new MouseOutCommand(this.getSelenium());
                break;
            ...
            case READ_ATTRIBUTE:
                command = new ReadAttributeCommand(this.getSelenium());
                break;

            default:
                return failResult(metadata, actionType, "Unsupported WebActionType for WebElement: '"
                        + actionType + "'");
            }
            ...
            String errorMessage = "Could not execute "
                    + actionType + " on WebElement '" + metadata.getName().getValue() + "'. Cause: " + ex.getMessage();
            ...
            result.setErrorMessage("Could not execute "
                    + actionType + " on WebElement '" + metadata.getName().getValue() + "'. Cause: " + ex.toString());
            result.setActionStatus(ActionStatusType.FAILED);
            return result;
        } finally {
    ...
    protected void validateProperties(PropertyList propertyList, WebActionType actionType) {
        ...
        case READ:
            if (propertyList.getPropertyList().size() < 1
                ...
                throw new IllegalArgumentException("Read requires 1 Property of Type Text");
            }
            break;
        case IS_AVAILABLE:
            if (propertyList == null
                    || propertyList.getPropertyList().isEmpty()
                    || PropertyHelper.getFromList(propertyList, PropertyType.BOOLEAN) == null) {
                throw new IllegalArgumentException("IsAvailable requires 1 Property of Type Boolean");
            }
            break;
        case READ_ATTRIBUTE:
            if (propertyList == null
                    || propertyList.getPropertyList().isEmpty() || propertyList.getPropertyList().size() < 2) {
                throw new IllegalArgumentException("ReadAttribute requires 2 Properties of Type Text");
            }
            break;
        }
    }
