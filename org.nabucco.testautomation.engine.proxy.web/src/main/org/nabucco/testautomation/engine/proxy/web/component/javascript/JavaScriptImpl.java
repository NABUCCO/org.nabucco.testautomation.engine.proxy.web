...
public class JavaScriptImpl extends AbstractWebComponent {
    ...
    protected ActionResponse internalExecute(TestContext context, PropertyList propertyList, Metadata metadata,
        ...
        try {
            ...
            default:
                return failResult(metadata, actionType, "Unsupported WebActionType for JavaScript: '"
                        + actionType + "'");
            }
            ...
            String errorMessage = "Could not execute "
                    + actionType + " on JavaScript '" + metadata.getName().getValue() + "'. Cause: " + ex.getMessage();
            ...
            result.setErrorMessage("Could not execute "
                    + actionType + " on JavaScript '" + metadata.getName().getValue() + "'. Cause: " + ex.toString());
            result.setActionStatus(ActionStatusType.FAILED);
            return result;
        } finally {
        ...
        case EVALUATE: {
            ...
            if (propertyList.getPropertyList().size() == 1) {
                ...
                if (property.getType() != PropertyType.TEXT
                        || property.getName().getValue().equals(EvaluateJavaScriptCommand.JAVA_SCRIPT)) {
                    throw new IllegalArgumentException("Evaluate requires 1 Property of Type String for the result");
                }
                ...
                if (p1.getType() == PropertyType.TEXT
                        && p1.getName().getValue().equals(EvaluateJavaScriptCommand.JAVA_SCRIPT)
                        && p2.getType() == PropertyType.TEXT) {
                    return;
                }

                if (p2.getType() == PropertyType.TEXT
                        && p2.getName().getValue().equals(EvaluateJavaScriptCommand.JAVA_SCRIPT)
                        && p1.getType() == PropertyType.TEXT) {
                    return;
                }
                throw new IllegalArgumentException("Evaluate requires 1 Property of Type String for the result");
            } else {
