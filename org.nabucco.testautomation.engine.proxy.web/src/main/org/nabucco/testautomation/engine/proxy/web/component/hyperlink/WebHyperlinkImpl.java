...
public class WebHyperlinkImpl extends AbstractWebComponent {
    ...
    protected ActionResponse internalExecute(TestContext context, PropertyList propertyList, Metadata metadata,
        ...
        try {
            ...
            default:
                return failResult(metadata, actionType, "Unsupported WebActionType for WebHyperlink: '"
                        + actionType + "'");
            }
            ...
            String errorMessage = "Could not execute "
                    + actionType + " on WebHyperlink '" + metadata.getName().getValue() + "'. Cause: "
                    + ex.getMessage();
            ...
            result.setErrorMessage("Could not execute "
                    + actionType + " on WebHyperlink '" + metadata.getName().getValue() + "'. Cause: " + ex.toString());
            result.setActionStatus(ActionStatusType.FAILED);
            return result;
        } finally {
