...
/**
 * Implementation of selenium actions for a documents like PDF, Excel, etc.
 * 
 * @author Steffen Schmidt, PRODYNA AG
 */
public class WebDocumentImpl extends AbstractWebComponent {

    private static final long serialVersionUID = 1L;

    /**
     * Creates a new {@link WebDocumentImpl} instance.
     * 
     * @param selenium
     *            the selenium instance.
     */
    public WebDocumentImpl(Selenium selenium) {
        super(selenium);
    }
    ...
    protected ActionResponse internalExecute(TestContext context, PropertyList propertyList, Metadata metadata,
        ...
        try {
            ...
            case READ:
                command = new ReadDocumentCommand(this.getSelenium());
                break;

            default:
                return failResult(metadata, actionType, "Unsupported WebActionType for WebDocument: '"
                        + actionType + "'");
            }
            ...
            result.setMessage("Executed WebDocument action='" + actionType + "'");
            ...
            String errorMessage = "Could not execute "
                    + actionType + " on WebDocument '" + metadata.getName().getValue() + "'. Cause: " + ex.getMessage();
            ...
            result.setErrorMessage("Could not execute "
                    + actionType + " on WebDocument '" + metadata.getName().getValue() + "'. Cause: " + ex.toString());
            result.setActionStatus(ActionStatusType.FAILED);
            return result;
        } finally {
